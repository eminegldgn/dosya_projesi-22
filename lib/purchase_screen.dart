...
void didChangeAppLifecycleState(
  ...
  state == AppLifecycleState.resumed &&
      _pendingMerchantOid != null
  ) {
    _checkPendingPaytrPayment();
  }
}

Future<void> _checkPendingPaytrPayment() async {
  ...
  await _apiService.getPaytrPaymentStatus(
  ...
  if (result['paid'] == true) {
    ...
    setState(() {
      ...
      _usedFreeTrialInThisPurchase = false;
      _alreadyPurchased = false;
      _fileUrl = fileUrl;
      _resultMessage =
      'Ödeme başarılı. PDF erişiminiz hazır.';
      _purchaseDate =
  ...
  if (status == 'failed') {
    final String message =
        result['failure_reason']?.toString() ??
...
String get _title {
return widget.note['title']?.toString() ?? 'Başlıksız Not';
}
...
String get _course {
return widget.note['course_name']?.toString() ??
'Ders belirtilmemiş';
}
...
String get _pdfName {
final String? filePath = widget.note['file_path']?.toString();
...
// =========================================================
  ...
  Future<void> _purchaseNote() async {
    ...
    try {
      ...
      // -------------------------------------------------------
      // 3. ÖNCE ÜCRETSİZ / ÖNCEDEN SATIN ALMA KONTROLÜ
      // -------------------------------------------------------
      ...
      // purchaseNote başarılı olduysa:
      // - ücretsiz hak kullanılmış olabilir
      // - veya not daha önce satın alınmış olabilir.
      if (purchaseSuccess) {
        final String message =
            purchaseResult['message']
                ?.toString() ??
                'PDF erişiminiz hazır.';

        final String? filePath =
        purchaseResult['filePath']
            ?.toString();
      ...
      final String iframeUrl =
          paymentResult['iframe_url']
              ?.toString() ??
              '';
      final String merchantOid =
          paymentResult['merchant_oid']
              ?.toString() ??
              '';

      if (merchantOid.isEmpty) {
        const String message =
            'Ödeme sipariş numarası alınamadı.';
      ...
      if (iframeUrl.isEmpty) {
        const String message =
            'PayTR ödeme bağlantısı alınamadı.';
      ...
      if (mounted) {
        setState(() {
          _resultMessage =
          'PayTR ödeme ekranı açıldı.';
        });
      }
    } catch (error) {
      debugPrint(
        'Satın alma / PayTR hatası: $error',
      );
...
_showMessage('PDF açılırken bir hata oluştu.');
}
}
...
onPressed: _isLoading ? null : _purchaseNote,
icon: _isLoading
? const SizedBox(
width: 19,
height: 19,
